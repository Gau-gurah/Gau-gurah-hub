import type { Category, Solution } from "@gau-gurah/types";
...
export function getSolutionStats() {
  const total = solutions.length;
  const active = solutions.filter((s) => s.status === "active").length;
  const beta = solutions.filter((s) => s.status === "beta").length;
  const upcoming = solutions.filter((s) => s.status === "coming-soon").length;
  const categoryCount = categories.length;
  return { total, active, beta, upcoming, categoryCount };
}

export function getCategoriesWithCount() {
  return categories.map((cat) => ({
    ...cat,
    count: solutions.filter((s) => s.category === cat.id).length,
  }));
}

export function getActiveSolutions() {
  return solutions.filter((s) => s.status !== "coming-soon");
}

export function getRelatedSolutions(slug: string) {
  const solution = getSolutionBySlug(slug);
  if (!solution) return [];
  return solutions.filter(
    (s) => s.category === solution.category && s.slug !== slug,
  );
}
